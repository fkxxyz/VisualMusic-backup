...
class CMp3Decoder : public CAudioDecoderTemplate<CHARTYPE> {
 ...
 private:
  ...
  static mad_flow mad_output_func(void *data, struct mad_header const *header,
                                  struct mad_pcm *pcm);
  static mad_flow mad_error_func(void *data, struct mad_stream *stream,
...
template <class CHARTYPE>
int CMp3Decoder<CHARTYPE>::Decode(CHARTYPE *filepath,
                                  CPCMStream<double> *pcm) {
  ...
  mad_decoder_init(&decoder, &tmp_data, mad_input_func, NULL, NULL,
                   mad_output_func, mad_error_func, NULL);
  ...
  pcm->m_max_amplitude_ = max_value;
  pcm->m_origin_pos_ = 0.0l;
  for (int n = 0; n < 2; n++) {
    pcm->m_samples_[n] = new double [tmp_data.data[0].size()];
    register double *p = pcm->m_samples_[n];
    register int i_end = pcm->m_num_samples_;
...
template <class CHARTYPE>
mad_flow CMp3Decoder<CHARTYPE>::mad_input_func(void *data,
  ...
  // 将数据送入 mad 流，去解码
  mad_stream_buffer(stream, (unsigned char *)tmp_data.mp3_data,
                    (unsigned long)(count + (p - tmp_data.mp3_data)));
...
template <class CHARTYPE>
mad_flow CMp3Decoder<CHARTYPE>::mad_header_func(
    void *data,
...
template <class CHARTYPE>
mad_flow CMp3Decoder<CHARTYPE>::mad_output_func(
    void *data, struct mad_header const *header, struct mad_pcm *pcm) {
  struct _TMP_DATA<CHARTYPE> &tmp_data = *(struct _TMP_DATA<CHARTYPE> *)data;

  if (tmp_data.pcm->m_num_samples_ == 0) {
    // 第一次读取到 mp3 信息

    tmp_data.pcm->m_num_samples_per_sec_ = pcm->samplerate;
    tmp_data.pcm->m_num_channels_ = pcm->channels;
  }

  // 更新大小
  tmp_data.pcm->m_num_samples_ += pcm->length;
  ...
  assert(tmp_data.pcm->m_num_samples_ == tmp_data.data[0].size());
  assert(tmp_data.pcm->m_num_samples_ == tmp_data.data[1].size());

  return MAD_FLOW_CONTINUE;
}

template <class CHARTYPE>
mad_flow CMp3Decoder<CHARTYPE>::mad_error_func(
